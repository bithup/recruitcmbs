...
/*              String signType = map.get("sign_type") + "";
                ...
                if (!"".equals(transStatus) && "1".equals(transStatus)) {
                    ...
                    Recharge recharge = rechargeService.getRechargeByOrderNo(order_no);
                    recharge.setUpdateDate(new Date());
                    recharge.setTradeNo(order_no);//交易号
    ...
    @RequestMapping(value="getTracNo")
    public void getTracNo(){
        Map<String,Object> resultMap = new HashMap<String, Object>();
        String trac_no = "TN"+DateTimeUtil.getDateTime();
        if(null == trac_no||"".equals(trac_no)){
            resultMap = getResultMap("0","获取商户系统跟踪号失败！");
        }else{
            resultMap = getResultMap("1","获取商户系统跟踪号成功！",trac_no);
        }
        outJson(resultMap);
    }
}
